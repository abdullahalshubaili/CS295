<?php
	...
	switch (@$_POST["flag"]){
			...
			switch ($week){
				case 'Monday':
					for($i = $start_time; $i < $end_time; $i++){
						if(!@$_SESSION['Monday'][$i] || @$_SESSION['Monday'][$i]['course_name'] == ''){
							@$_SESSION['Monday'][$i]['course_name'] = $course_name;
						}else{
							array_push($error_cs, $i);
						}
					}
					if(count($error_cs) > 0){
						$Error = "";
						foreach($error_cs as $tmp){
							$Error.=$tmp.", ";
						}
				...
				case 'Tuesday':
					for($i = $start_time; $i < $end_time; $i++){
						if(!@$_SESSION['Tuesday'][$i] || @$_SESSION['Tuesday'][$i]['course_name'] == ''){
							@$_SESSION['Tuesday'][$i] = array('course_name' => $course_name);
						}else{
							array_push($error_cs, $i);
						}
					}
				...
				case 'Wednesday':
					for($i = $start_time; $i < $end_time; $i++){
						if(!@$_SESSION['Wednesday'][$i] || @$_SESSION['Wednesday'][$i]['course_name'] == ''){
							@$_SESSION['Wednesday'][$i] = array('course_name' => $course_name);
						}else{
							array_push($error_cs, $i);
						}
					}
		...
		print "<SCRIPT>alert('you cannot register in $Error!');</SCRIPT>";
				...
				case 'Thursday':
					for($i = $start_time; $i < $end_time; $i++){
						if(!@$_SESSION['Thursday'][$i] || @$_SESSION['Thursday'][$i]['course_name'] == ''){
							@$_SESSION['Thursday'][$i] = array('course_name' => $course_name);
						}else{
							array_push($error_cs, $i);
						}
					}
			...
			print "<SCRIPT>alert('you cannot register in $Error!');</SCRIPT>";
				...
				case 'Friday':
					for($i = $start_time; $i < $end_time; $i++){
						if(!@$_SESSION['Friday'][$i] || @$_SESSION['Friday'][$i]['course_name'] == ''){
							@$_SESSION['Friday'][$i] = array('course_name' => $course_name);
						}else{
							array_push($error_cs, $i);
						}
					}
		...
		case 'remove_course':
			...
			$session_val = explode(" ",$course_id);
			unset($_SESSION[$session_val[0]][$session_val[1]]);
...
?>
				...
				<h1>Weekly Schedule for the CS Lab Winter 2014</h1>
...
<?php
print '<form action="add_course.php" method="post">
<p><input type="submit" name="submit" value="Add course" /></p>
	</form>';
print '<form action="remove_course.php" method="post">
<p><input type="submit" name="submit" value="Remove Course" /></p>
	</form>';
